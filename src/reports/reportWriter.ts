...
export async function writeReports(report: SessionReport): Promise<void> {
  ...
  await writeFile(path.join(report.sessionDir, "summary.md"), generateSummaryMarkdown(report), "utf8");
  await writeFile(path.join(report.sessionDir, "commands.md"), generateCommandsMarkdown(report), "utf8");
